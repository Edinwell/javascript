...
var json = '{"x":[100,200,300,100,200,300],"y":[100,100,100,200,200,200]}';
...
function Ball(x, y, speedRate) {
	...
	var base = Math.sqrt(Math.pow(speedX, 2) + Math.pow(speedY, 2)) * 0.08;
...
Ball.prototype = {
	...
	collision : function(otherBall) {
		...
		if (dist < radius) {
			...
			resultDX = this.dX * cos + this.dY * sin;
			resultDY = this.dY * cos - this.dX * sin;
...
function generateBall() {
	var xyInfo = JSON.parse(json);
	for (var i = 0; i < 3; i++) {
		ballsArray[ballCount] = new Ball(xyInfo.x[ballCount], xyInfo.y[ballCount], 1);
		ballsArray[ballCount].display();
		ballCount++;
	}

	for (var i = 0; i < 3; i++) {
		ballsArray[ballCount] = new SuperBall(xyInfo.x[ballCount], xyInfo.y[ballCount], 2);
		ballsArray[ballCount].display();
		ballCount++;
	}
